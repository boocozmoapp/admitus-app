    ...
    return NextResponse.json(
      {
        ...
        message: emailResult.delivered
          ...
          : "Account created, but email delivery is not configured in this environment.",
      },
      { status: 201 }
    );
  } catch (error) {
    console.error("[admitus] Registration failed", error);
    return NextResponse.json(
      { error: "We could not create the account right now. Please try again in a moment." },
      { status: 500 }
    );
  }
}
